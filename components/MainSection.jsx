...
class MainSection extends Component {
  constructor(props, context) {
    ...
    this.updateOverallList = this.updateOverallList.bind(this);
    this.updateRegNo = 0;
  ...
  checkInSubmit() {
    ...
    if (index !== -1) {
      ...
      fetch(`${baseUrl}/checkin`, {
        ...
        fetch(`${baseUrl}/checkinlist`).then((response) => response.json()).then((data) => {
          const compare = (a, b) => {
  ...
  render() {
    ...
    const keysForTable = Object.keys(TABLE_DATA[0]).filter((value) => !["months", "FATHER", "Date of Joining", "Gender"].includes(value));
    const { NAME = "", lastCheckInTime = "", rowColor = "", expiredDays = "", monthlyAttendance = "" } = this.state.LastCheckInPerson;
    return (
      <section className="main" style={defaultStyle}>
        <Tabs style={{ backgroundColor: "#00695c" }}>
          <Tab
        ...
        {!addNewSection && showAttendance &&
          <div>
            ...
            <RaisedButton
              ...
              label="Check In"
              secondary={true}
              onClick={this.checkInSubmit}
            />
            <div style={style.flexContainer}>
              {this.state.checkinTable.map((row, index) => <Chip
        ...
        <Dialog
          ...
          bodyStyle={style.DialogStyle}
          titleStyle={style.DialogTitle}
          actionsContainerStyle={style.DialogStyle}
          actions={[<FlatButton
            label="Cancel"
            primary={true}
        ...
        ><div style={style.DialogStyle}>
            <h1>{`Welcome ${this.state.LastCheckInPerson['NAME']} --> ${this.state.LastCheckInPerson["Reg No:"]}`}</h1>
            <h2>{`CheckIn Time ${this.state.LastCheckInPerson['lastCheckInTime']}`}</h2>
            {this.state.LastCheckInPerson['rowColor'] === "#f47979" ? <h3 style={{ backgroundColor: "#ff0026", color: "z", padding: "10px", fontSize: "16px" }}>{`Your ${this.state.LastCheckInPerson['Typeof pack']} Package expired since ${expiredDays} Days, last paid date -->${new Date(this.state.LastCheckInPerson['DUE DATE']).toDateString()}`}</h3> :
              <h1 style={{ backgroundColor: "#00ff0c", color: "#888988", padding: "10px", fontSize: "16px" }}>{`Your ${this.state.LastCheckInPerson['Typeof pack']} Package going to expire in ${this.state.LastCheckInPerson['expiredDays']} Days, Due Date -->${new Date(this.state.LastCheckInPerson['DUE DATE']).toDateString()}`}</h1>}
